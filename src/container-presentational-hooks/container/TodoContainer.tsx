...
export const TodoContainer: React.FC = () => {
    ...
    const handleFormTextChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const handleFormSubmit = (event: FormEvent<HTMLFormElement>) => {
        const todo = {id: new Date().getTime(), text: text, status: TodoStatus.ACTIVE};
     ...
     * @param id TODOのID
     */
    const handleActiveButtonClick = (id: number) => {
        const todo = todoList.find(todo => todo.id === id) as Todo
    ...
    const filterTodoList = () => {
        ...
        return todoList.filter(todo => todo.status.toString() === filter.toString())
    }
